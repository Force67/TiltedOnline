...
TiltedOnlineApp::TiltedOnlineApp()
{
    ...
    std::error_code ec;
    std::filesystem::create_directory(TiltedPhoques::GetPath() / "logs", ec);

    auto pathBitch = TiltedPhoques::GetPath() / "logs/tp_client.log";
    auto rotatingLogger = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(pathBitch.u8string(), 1048576 * 5, 3);
